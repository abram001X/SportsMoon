...
export function MenuGames({ apiCopaAmerica, apiEuroCopa }) {
  ...
  const gamesPast = (copaSelect === apiCopaAmerica ? copaSelect : apiEuroCopa) .filter((elements) => {
      ...
      {gamesPast.map((elements, j) => {
        return (
          <article className="cont_games-child-menugames" key={j}>
            ...
            />
            <p className={elements.teams.home.winner ? winner : loser}>{elements.teams.home.name.slice(0, 3)}</p>
            ...
            <img
              ...
              className="img-child-menugames"
            />
            <p className={elements.teams.away.winner ? winner : loser}>{elements.teams.away.name.slice(0, 3)}</p>
